...
for (my $i = 0; $i < scalar @IN2; $i++){
  ...
  for (my $j = 0; $j < scalar @IN; $j++){
    ...
    $res_pos = @inrow[4];
    #$KLalt = $KL+rand(1)*$KL;
    if($res_pos == $res_pos1){print "$atom_pos\t"."$res_pos1\t"."$res_type1\t"."$KL\t"."$p_value\t"."$chainID\n";}
    if($res_pos == $res_pos1){print OUTFILE "$atom_pos\t"."$res_pos1\t"."$res_type1\t"."$KL\t"."$p_value\t"."$chainID\n";}
    if($res_pos == $res_pos1){print OUTFILE2 "$KL\n";}
    if($res_pos == $res_pos1){print OUTFILE3 "$p_value\n";}
  }
}
 print OUTFILE2 "\n";
 print OUTFILE3 "\n"; 
